    ...
    def _build_act_prompt(self, task: str, context: dict[str, Any]) -> str:
        ...
        if is_iteration and prev_code:
            ...
            prompt += (
                ...
                "Before outputting, MENTALLY TEST the result: does the fix actually solve the reported problem? "
                "If the user reports a character falling through the ground, check collision detection. "
            ...
            prompt += (
                ...
                "- Character MUST have at minimum: idle animation, walk animation, jump animation using sprite sheets or CSS sprites\n"
                "- If requesting a clone of a known game (Mario, Pac-Man, etc), replicate its core mechanics faithfully\n"
                "- Levels/stages with clear progression\n"
                "- Score display, lives/health, controls instructions visible on screen\n"
                "- Start screen with game title and 'Press Space to Start'\n"
                "- Game over screen with score and 'Press Space to Restart'\n"
                "- Use document.addEventListener for key events (iframe compat)\n"
                "- Add tabindex='0' to game container and .focus() on start\n"
